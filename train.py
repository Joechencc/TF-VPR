...
def train(scene_index):
    ...
    if not os.path.exists("./generating_queries/train_pickle"):
        os.mkdir("./generating_queries/train_pickle")
    if not os.path.isdir(cfg.RESULTS_FOLDER):
        os.mkdir(cfg.RESULTS_FOLDER)
    generate_dataset_tt.generate(cfg.BASE_PATH, scene_index, 0, inside=False)
    ...
    if cfg.LOSS_FUNCTION.lower() == 'quadruplet':
        loss_function = PNV_loss.quadruplet_loss
    elif cfg.LOSS_FUNCTION.lower() == 'triplet_ri':
    ...
    for epoch in range(starting_epoch, cfg.MAX_EPOCH):
        ...
        generate_dataset_tt.generate(cfg.BASE_PATH, scene_index, epoch, definite_positives=None, inside=False)
        ...
        eval_recall_1, eval_recall_5, eval_recall_10 = \
                evaluate.evaluate_model(model, optimizer, epoch,
                                        scene_index, True)
...
def train_one_epoch(model, optimizer, train_writer, loss_function, epoch,
                    scene_index, TRAINING_QUERIES, TEST_QUERIES, DB_QUERIES):
    ...
    for i in range(len(train_file_idxs)//cfg.BATCH_NUM_QUERIES):
        ...
        for j in range(cfg.BATCH_NUM_QUERIES):
            if (len(TRAINING_QUERIES[batch_keys[j]]["positives"]) < cfg.TRAIN_POSITIVES_PER_QUERY):
                print("len(TRAINING_QUERIES[batch_keys[j]][positives]:" + \
                      str(len(TRAINING_QUERIES[batch_keys[j]]["positives"])))
                print("cfg.TRAIN_POSITIVES_PER_QUERY:" + \
                      str(cfg.TRAIN_POSITIVES_PER_QUERY))
                assert(0)
                faulty_tuple = True
                break
            ...
            if (len(TRAINING_LATENT_VECTORS) == 0):
                q_tuples.append(
                    get_query_tuple(TRAINING_QUERIES[batch_keys[j]],
        ...
        loss = loss_function(output_queries, output_positives,
                             output_negatives,  0.1,
...
if __name__ == "__main__":
    for i in range(cfg.SCENE_INDEX):
        train(i)
